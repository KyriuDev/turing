...
int try_domino(int dom){
	...
	while (pos_haut + strlen((char*) domsu[dom]) > taille_mot - 5) {
	   	grow_mots();
	}
	
	while (pos_bas + strlen((char*) domsv[dom]) > taille_mot - 5) { 
		grow_mots();
	}
	...
	if(strncmp((char*) mot_haut, (char*) mot_bas, minimum(pos_haut + strlen((char*) domsu[dom]), pos_bas + strlen((char*) domsv[dom]))) == 0) {
		ret = 1;
	}
...
int is_not_finished(){
	if(pos_haut > 0 && pos_bas > 0 && pos_haut == pos_bas) {
	   	return 0;
	}
...
int main(int argc, char* argv[]){
	...
	while(is_not_finished() && (i++ < MAX_RUNS)) {
		add_domino(find_domino());
	}
